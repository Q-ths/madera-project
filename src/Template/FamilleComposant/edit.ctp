<div ng-controller="FamilleComposantController">
    ...
    <div class="row title-page">
        <div class="col-10 col-sm-10 col-md-10 col-lg-10 col-xl-10 title">
            <h4>Modification d'une famille de composant</h4>
        </div>
        ...
        <div class="row">
            <div class="col-6 col-sm-6 col-md-6 col-lg-6 col-xl-6">
                <div class="form-group">
                    <label >Libelle *</label>
                    <input type="text" class="form-control" ng-class="(familleComposant.nom != null) ? 'is-valid' : 'is-invalid' " id="tva-name" ng-model="familleComposant.nom" placeholder="Libelle *">
                </div>
            </div>
        </div>
    </div>
...
<script>
    angular.module('tva', [])
        .controller('FamilleComposantController', ['$scope','$http', function ($scope,$http) {
            ...
            $scope.familleComposant = {};

            $http.get('/famille-composant/view/' + window.location.search.split('id=')[1]).then(function ($response) {
                $scope.familleComposant = $response.data;
            });

            $scope.add = function () {
                let size = Object.keys($scope.familleComposant).length;
                if(size < 1) {
                ...
                fetch('/famille-composant/edit/' + $scope.familleComposant.id, {headers:header, method:'post', body:JSON.stringify($scope.familleComposant)})
                    .then(function ($response) {
                        $('#alert-info-saving').hide();
                        window.location.href = '/famille-composant';
                    });
            }
        }]);
