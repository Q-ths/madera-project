<div ng-controller="TypeFinitionController">
    ...
    <div class="row title-page">
        <div class="col-10 col-sm-10 col-md-10 col-lg-10 col-xl-10 title">
            <h4>Modification d'une finition</h4>
        </div>
        ...
        <div class="row">
            <div class="col-6 col-sm-6 col-md-6 col-lg-6 col-xl-6">
                <div class="form-group">
                    <label >Libelle *</label>
                    <input type="text" class="form-control" ng-class="(finition.nom != null) ? 'is-valid' : 'is-invalid' " id="tva-name" ng-model="finition.nom" placeholder="Libelle *">
                </div>
            </div>
        </div>
    </div>
...
<script>
    angular.module('finition', [])
        .controller('TypeFinitionController', ['$scope','$http', function ($scope,$http) {

            $(".alert").hide();

            $scope.finition = {};

            $http.get('/type-finition/view/' + window.location.search.split('id=')[1]).then(function ($response) {
                $scope.finition = $response.data;
            });

            $scope.add = function () {
                let size = Object.keys($scope.finition).length;
                if(size < 1) {
                ...
                fetch('/type-finition/edit/' + $scope.finition.id, {headers:header, method:'post', body:JSON.stringify($scope.finition)})
