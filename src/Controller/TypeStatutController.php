 ...
 *
 * @property TypeStatutTable $TypeStatut
 */
class TypeStatutController extends AppController
{
    ...
    public function get()
    {
        $type_isolants = $this->TypeStatut->find('all')->toArray();

        return $this->renderToJson(json_encode($type_isolants));
    }

    public function getEnable() {
        $type_isolants = $this->TypeStatut->find('enable')->toArray();
    ...
    {
        $type_isolant = $this->TypeStatut->get($id);
    ...
    public function create()
    {
        ...
        $type_isolant = $this->TypeStatut->newEntity();
        if ($this->request->is('post')) {
            $type_isolant = $this->TypeStatut->patchEntity($type_isolant, $this->request->getData());
            $type_isolant->date_in = (new \DateTime())->format('Y-m-d H:m:s');
            if ($this->TypeStatut->saveOrFail($type_isolant)) {
                /** @var array $type_isolant */
                $type_isolants = $this->TypeStatut->find('OrderByActivate')->toArray();
                return $this->renderToJson(json_encode($type_isolants));
            }
            return $this->response->withStatus(400);
        }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->TypeStatut->save($type_isolant)) {
            $type_isolants = $this->TypeStatut->find('OrderByActivate')->toArray();
            return $this->renderToJson(json_encode($type_isolants));
        } else {
            return $this->response->withStatus(400);
